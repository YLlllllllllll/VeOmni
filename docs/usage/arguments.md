...
| cross_entropy_loss_release_cache | `bool` | `false` | Synchronize and release transient chunk-loss allocator cache after precomputed gradients are packed. This can lower peak memory before model backward on constrained profiles, at the cost of per-microbatch synchronization and allocator churn. It only affects `chunk_loss`/`npu`. |
...
| enable_async_activation | `bool` | `False` | Enable async activation offload via stream-based D2H/H2D. Mutually exclusive with `enable_activation`. Requires `activation_offload_modules`, and at least one pattern must match. |
