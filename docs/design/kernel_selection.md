...
allocator-cache release after the chunked kernel returns. On the plain CE path
this runs after the outer saved-tensor hook has packed the precomputed
input/lm-head gradients; on the `return_log_probs` and top-k distillation
paths it runs in the dispatch shim after those chunked helpers return. It can
lower the peak memory carried into model backward on constrained accelerators,
but it may reduce throughput through synchronization and allocator churn. The
default is `false`; this setting has no effect on the `eager` or
`liger_kernel` implementations.

Note on activation offload: the chunked CE path installs a nested identity
`saved_tensors_hooks` around the short-lived per-chunk graph so vocab-sized
FP32 logits stay on device (offloading them would be pure transfer cost).
That deliberately excludes the largest transient from CPU activation offload;
