...
def test_npu_async_hdfs_validation_runs_on_unprofiled_rank(monkeypatch):
    ...
    monkeypatch.setattr(helper, "IS_NPU_AVAILABLE", True)
    monkeypatch.setattr(helper, "create_profiler", lambda **kwargs: pytest.fail("must not create a profiler"))

    with pytest.raises(ValueError, match="pod-local trace_dir"):
...
def test_npu_offline_auto_uploads_in_merlin(monkeypatch, tmp_path):
    fake_profiler = _FakeNpuProfiler()
    sidecar_calls = []
    raw_dir = tmp_path / "rank0_ascend_pt"
    raw_dir.mkdir()
