...
def test_chunk_loss_factory_returns_lazy_dispatch():
    """OpSlot factory must not snapshot release_cache into a partial."""
    ...
    assert kernel is cross_entropy_module._chunk_loss_dispatch


def test_chunk_loss_dispatch_resolves_release_cache_lazily(monkeypatch):
    """Later set_ops_config must be observed by the OpSlot/LOSS_MAPPING path."""
    captured = {}
    ...
    set_ops_config(SimpleNamespace(cross_entropy_loss_release_cache=False))
    try:
        kernel = cross_entropy_module._chunk_loss_causal_factory()
        kernel(
            hidden_states=torch.empty(1),
        ...
        assert captured["release_cache"] is False
        ...
        assert captured["release_cache"] is True
    finally:
        set_ops_config(previous)


def test_chunk_loss_dispatch_releases_cache_on_logprobs_path(monkeypatch):
    """return_log_probs / distill paths must honor release_cache too."""
    events = []

    def fake_chunk_logprobs_function(*args, **kwargs):
        events.append("logprobs")
        return torch.zeros(1, 2), torch.zeros(1, 2)

    def fake_release(device):
        events.append(("release", device.type))

    monkeypatch.setattr(cross_entropy_module, "chunk_logprobs_function", fake_chunk_logprobs_function)
    monkeypatch.setattr(cross_entropy_module, "_release_accelerator_cache", fake_release)

    loss, logits, aux = cross_entropy_module._chunk_loss_dispatch(
        hidden_states=torch.empty(1, 2, 4),
        weights=torch.empty(7, 4),
    ...
    assert isinstance(aux, FusedLinearAuxOutput)
    assert events == ["logprobs", ("release", "cpu")]


def test_chunk_loss_dispatch_releases_cache_on_distill_path(monkeypatch):
    """Top-k distillation path must honor release_cache too."""
    events = []

    def fake_chunk_topk_distill_function(*args, **kwargs):
    ...
    monkeypatch.setattr(cross_entropy_module, "chunk_topk_distill_function", fake_chunk_topk_distill_function)
    monkeypatch.setattr(cross_entropy_module, "_release_accelerator_cache", fake_release)
    ...
    loss, logits, aux = cross_entropy_module._chunk_loss_dispatch(
        ...
        labels=torch.zeros(1, 2, dtype=torch.long),
        return_log_probs=True,
        teacher_topk_ids=torch.zeros(1, 2, 3, dtype=torch.long),
    ...
    assert loss is None and logits is None
    assert isinstance(aux, FusedLinearAuxOutput)
    assert events == ["distill", ("release", "cpu")]
